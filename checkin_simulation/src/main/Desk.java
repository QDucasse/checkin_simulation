...
public class Desk extends Observable implements Runnable {
	 ...
	 * 
	 * @param passengerQueue The queue holding all the passengers waiting to
	 *                       check-in.
	 * @param processingTime The time this desk takes to process one passenger.
	 */
	 ...
	 * 
	 * @return deskNumber Number of the desk.
	 */
	 ...
	 * 
	 * @return currentPassenger Passenger being checked in.
	 */
	...
	/**
	 * While the queue is not empty, the desk will accept passengers for check-in.
	 * Each time a passenger is accepted, the desk will process it for a certain
	 * time (processing time ± 2s).
	 */
	public void run() {
		while (!passengerQueue.isEmpty()) {
			try {
				...
				if (currentPassenger != null) {
					...
					// The thread will sleep for a duration between 1 second (3000 - 2000) and 5
					// seconds (3000 + 2000)
			...
			} catch (InterruptedException | IOException | NullDimensionException | NegativeDimensionException e) {
				e.printStackTrace();
			}
		}
	...
	/**
	 * Method to associate random baggage dimension to every passenger in the
	 * passenger list Dimensions are included between 1 and 200
	 * 
	 * @throws NegativeDimensionException Baggage dimensions should not be negative.
	 * @throws NullDimensionException     Baggage dimensions should not be null.
	 */

	public void setRandomBaggageToPassenger(Passenger targetPassenger)
			throws NegativeDimensionException, NullDimensionException {
		Random random = new Random();
	...
	/**
	 * Method used for the checkIn button Set the check-in value to true if the
	 * passenger last name and booking reference match. Set a baggage for a
	 * passenger, display excess fee if applied.
	 */
